...
def predict_24_hours_hours_ahead(region_id = "PJM"):
    ...
    # Some regions don't report every fuel type (e.g. no Geothermal); reindex
    # against the training feature order and fill missing columns with 0 so
    # the column layout always matches what the model was trained on
    live_raw_matrix = historical_df.reindex(columns=feature_cols, fill_value=0).ffill().bfill().values

    # Scales live data into the same [0, 1] range the model was trained on
    ...
    # NOTE: "Other" is treated as fossil (920 lbs/MWh, same as Natural Gas) as an
    # EIA proxy assumption. For CISO specifically, "Other" also absorbs Geothermal
    # generation (see the merge in api_wrapper.py) — actual Geothermal, Nuclear,
    # and CO2 intensity are effectively 0, so this overstates CISO's carbon
    # intensity somewhat. Known limitation, not corrected here.
    co2_factors = {
        "Coal": 2210.0,
        "Natural Gas": 920.0,
        "Petroleum": 2110.0,
    ...
    hourly_co2_tons = hourly_co2_lbs / 2204.62  # 1 Metric Ton = 2204.62 lbs
